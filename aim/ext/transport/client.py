...
class Client:
    ...
    def _separate_paths(self, remote_path):
        if len(remote_path.split(":")) < 3:
            ...
            creds = parts[0]
            hostname, port_sub_path = parts[1].rsplit(":", maxsplit=1)
            try:
                port, sub_path = port_sub_path.split("/", maxsplit=1)
            except ValueError:  # no sub_path specified
                port = port_sub_path
                sub_path = ""
            self._remote_path = f"{creds}@{hostname}:{port}"
            self._sub_path = sub_path
    ...
    def _create_remote_channels(self):
        ...
        msg_max_size = int(
            os.getenv(AIM_RT_MAX_MESSAGE_SIZE, AIM_RT_DEFAULT_MAX_MESSAGE_SIZE)
        )
        options = [
        ...
        if ssl_certfile:
            ...
            self._remote_router_channel = grpc.secure_channel(
                self.remote_path, root_certificates, options=options
            )
        else:
            self._remote_router_channel = grpc.insecure_channel(
                self.remote_path, options=options
            )
        self._remote_router_stub = remote_router_pb2_grpc.RemoteRouterServiceStub(
            self._remote_router_channel
        )
        ...
        if self._remote_worker_address == self.remote_path:
            ...
            if ssl_certfile:
                self._remote_channel = grpc.secure_channel(
                    self._remote_worker_address, root_certificates, options=options
                )
            else:
                self._remote_channel = grpc.insecure_channel(
                    self._remote_worker_address, options=options
                )

        self._remote_stub = remote_tracking_pb2_grpc.RemoteTrackingServiceStub(
    ...
    def reinitialize_resource(self, handler):
        ...
        self.get_resource_handler(
            resource, resource.resource_type, handler, resource.init_args
        )
    ...
    def _check_remote_version_compatibility(self):
        ...
        if client_version < remote_version:
            raise RuntimeError(
                error_message_template.format(remote_version, client_version)
            )
    ...
    def _get_worker_port_offset(self):
        request = router_messages.ConnectRequest(client_uri=self.uri)
        response = self._remote_router_stub.connect(
    ...
    def client_heartbeat(self):
        ...
        response = self._remote_router_stub.client_heartbeat(
            request, metadata=self._request_metadata
        )
        return response

    def reconnect(self):
        request = router_messages.ReconnectRequest(client_uri=self.uri)
        response = self._remote_router_stub.reconnect(
            request, metadata=self._request_metadata
        )
    ...
    def disconnect(self):
        request = router_messages.DisconnectRequest(client_uri=self.uri)
        response = self._remote_router_stub.disconnect(
            request, metadata=self._request_metadata
        )
    ...
    def get_version(
        ...
        response = self._remote_router_stub.get_version(
            request, metadata=self._request_metadata
        )
    ...
    def get_resource_handler(self, resource, resource_type, handler="", args=()):
        request = rpc_messages.ResourceRequest(
            resource_type=resource_type, handler=handler, client_uri=self.uri, args=args
        )
    ...
    def release_resource(self, queue_id, resource_handler):
        request = rpc_messages.ReleaseResourceRequest(
            handler=resource_handler, client_uri=self.uri
        )
        if queue_id != -1:
            self.get_queue(queue_id).wait_for_finish()
        response = self.remote.release_resource(
            request, metadata=self._request_metadata
        )
    ...
    def run_instruction(self, queue_id, resource, method, args=(), is_write_only=False):
        ...
        if is_write_only:
            ...
            self.get_queue(queue_id).register_task(
                self,
                self._run_write_instructions,
                list(encode_tree([(resource, method, args)], strict=False)),
            )
            return
    ...
    def _run_read_instructions(self, queue_id, resource, method, args):
        ...
        if queue_id != -1:
            self.get_queue(queue_id).wait_for_finish()
        resp = self.remote.run_instruction(
            message_stream_generator(), metadata=self._request_metadata
        )
        ...
        def message_stream_generator():
            for chunk in stream:
                yield rpc_messages.WriteInstructionsRequest(
                    version="0.1", client_uri=self.uri, message=chunk
                )

        response = self.remote.run_write_instructions(
            message_stream_generator(), metadata=self._request_metadata
        )
    ...
    def flush_instructions_batch(self, queue_id):
        ...
        self.get_queue(queue_id).register_task(
            self,
            self._run_write_instructions,
            list(encode_tree(self._thread_local.atomic_instructions)),
        )
        self._thread_local.atomic_instructions = None
